...
public class Arquivo<T extends EntidadeArquivo> {
    ...
    public T[] readAll() throws Exception {
        ...
        while (pos < arquivo.length()) {
            ...
            pos = pos + 3 + tamanho; // lapide (1) + short (2) + dados
        }

        @SuppressWarnings("unchecked")
        T[] array = (T[]) java.lang.reflect.Array.newInstance(construtor.getDeclaringClass(), lista.size());
        return lista.toArray(array);
    }

    public int getUltimoID() throws Exception {
        this.arquivo.seek(1); // posição onde o último ID é armazenado
