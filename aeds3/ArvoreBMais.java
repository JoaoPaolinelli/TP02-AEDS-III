...
public class ArvoreBMais<T extends RegistroArvoreBMais<T>> {
    ...
    public ArrayList<T> readAll() throws Exception {
        ...
        // Navega até a primeira folha
        while (ponteiro != -1) {
            arquivo.seek(ponteiro);
            byte tipo = arquivo.readByte();
    
            if (tipo == 0) break; // nó folha
    
            int qtd = arquivo.readInt();
    
            for (int i = 0; i < qtd; i++) {
                T dummy = construtor.newInstance();
                arquivo.skipBytes(dummy.size()); // ignora chave
                arquivo.skipBytes(8); // ignora ponteiro
            }
    
            ponteiro = arquivo.readLong(); // filho mais à esquerda
        }
    
        // Agora percorre as folhas
        while (ponteiro != -1) {
            arquivo.seek(ponteiro);
            byte tipo = arquivo.readByte();
    
            if (tipo != 0) break; // deveria ser folha
    
            int qtd = arquivo.readInt();
    
            for (int i = 0; i < qtd; i++) {
                T exemplo = construtor.newInstance();
                byte[] dados = new byte[exemplo.size()];
    
                if (arquivo.getFilePointer() + dados.length > arquivo.length()) {
                    System.out.println("Tentando ler além do fim do arquivo. Abortando leitura");
                    return todos;
                }
    
                arquivo.readFully(dados);
    
                T chave = construtor.newInstance();
                chave.fromByteArray(dados);
                todos.add(chave);
            }
